...
pub struct Server {
    messages: Mutex<HashMap<Identifier, Vec<Vec<u8>>>>,
    prekeys: Mutex<HashMap<Identifier, Vec<Vec<u8>>>>,
    // connections: Mutex<HashMap<Identifier, SplitSink<WebSocketStream<>, Message>>>,
}

impl Server {
    async fn run_connection<S>(&mut self, connection: WebSocketStream<S>)
    where
        S: AsyncRead + AsyncWrite + Unpin,
    {
        ...
        let sender = Identifier::Referenced(Vec::new());
    ...
    fn test_server(&mut self) -> Runtime {
